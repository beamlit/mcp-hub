...
from ...agents.types import MCPState, MCPStateRequestBuild, MCPStateResponseBuild
...
with open(os.path.join(current_dir, "prompt_python.md"), "r") as f:
    ...
    PROMPT_PYTHON = PROMPT_PYTHON.format(input=MCPStateRequestBuild.llm_format(), output=MCPStateResponseBuild.llm_format())
...
def after_agent_callback(callback_context: CallbackContext) -> Optional[types.Content]:
    try:
        ...
        mcp_build = MCPStateResponseBuild(**json.loads(callback_context.state["mcp_build_result"]))
        # When the LLM did not find any config in files it tends to return sample data from llm_format
        for key in json.loads(MCPStateResponseBuild.llm_format())["config"].keys():
            if key in mcp_build.config:
                del mcp_build.config[key]
        callback_context.state["mcp_build"] = mcp_build
        mcp_state.build = mcp_build
        print(f"{callback_context.state['name']} - Build component generated successfully")
        return types.Content(parts=[types.Part(text="Build component generated successfully")])
    except Exception as e:
        print(f"{callback_context.state['name']} - Error generating build component: {e}\nMCP Build state:")
        print(callback_context.state["mcp_build_result"])
        callback_context.state["mcp_state"].errors.append(f"Error generating build component: {e}")
        return None

python = LlmAgent(
    name="build_python",
    description="Build Agent responsible for buildPath, srcPath the project in python.",
    instruction=PROMPT_PYTHON,
    model=get_model_big(),
    tools=[read_file, list_directory],
    input_schema=MCPStateRequestBuild,
    output_key="mcp_build_result",
...
typescript = LlmAgent(
    ...
    description="Build Agent responsible for buildPath, srcPath the project in typescript.",
...
build = LlmAgent(
    ...
    description="Build Agent responsible for distPath, srcPath for the project.",
    instruction=PROMPT,
