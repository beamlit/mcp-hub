...
from .src.agents.types import MCPState
from .src.libs.mcppulse import PulseMcpClient, Server
...
async def fetch_mcp_server(repository: str, path: str = "."):
    async with PulseMcpClient() as client:
        query = repository.split("/")[-1]
        if path != ".":
            query = path.split("/")[-1]
        servers = await client.list_servers(query=query)
...
async def main():
    ...
    servers = []
    if args.repository:
        server = await fetch_mcp_server(args.repository, path=args.path)
        if server:
        ...
        with open("servers.yaml") as f:
            ...
            for _server in _servers:
                ...
                if _server.get("repository"):
                    tasks.append((fetch_mcp_server, _server["repository"], _server.get("path", ".")))
